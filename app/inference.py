...
def pred():
    ...
    date_value_array = scraped_output.replace('new Date(', '').replace(')', '').replace('[', '').replace(']',
                                                                                                         '').replace(
        '"', '').replace('/', '-').split(',')
    ...
    #sgd_reg = pickle.load(open(r"linear_reg_10_25.sav", 'rb'))
    # model_path = r'C:\Users\Parth Modi\Desktop\Data Science 1\bitcoin-price-forecasting-main\app\bilstm_26'
    sgd_reg = load_model('model_final.h5')
    ...
    today_btc_closing_price = sgd_reg.predict(X_scaled.values[-1].reshape(1,1,-1))
    final_df['Date'] = final_df['Date'].dt.strftime('%d-%m-%Y')
    return final_df, round(float(today_btc_closing_price))


def get_current_close_price():
    return investpy.get_crypto_historical_data(crypto='bitcoin', from_date='01/01/2021',
                                               to_date=datetime.today().strftime('%d/%m/%Y'))['Close'][-1]


def get_df():
    return investpy.get_crypto_historical_data(crypto='bitcoin', from_date='01/04/2013',
                                               to_date=datetime.today().strftime('%d/%m/%Y')).reset_index()[
        ['Date', 'Close']]
